...
public class InsertAppointmentCommand extends SQLCommand {

	private Appointment appointment;
	
	public InsertAppointmentCommand(Appointment appointment){
		this.appointment = appointment;
	}
	
	@Override
	void queryDB() throws Exception {
		String query = String.format("INSERT INTO %s.%s (advisor_id, student_id, start_time, end_time, student_notes, advisor_notes, status_id, task_id)"
				+ " VALUES (?,?,?,?,?,?,?,?)", config.DBNAME, Appointment.TABLE_NAME);
		try {
